...
DARKFLYEXEC = "DarkFly"
COMMAND = {
    "termux":{1:"cd "+TERMUX+"DarkFly",2:"python {}/{}".format(TERMUX,"DarkFly/main.py")},
    "linux":{1:"cd "+LINUX+"DarkFly",2:"python3 {}/{}".format(LINUX,"DarkFly/main.py")}
}
...
class Install:
    ...
    def installLinux(self):
        ...
        system("mv ../DarkFly %s && chmod +x %s/%s && chmod 777 -R %s"%(LINUX,LINUX,DARKFLYEXEC,LINUX+"/DarkFly"))
...
class Uninstall(Install):
    def uninstall(self):
        if self.environmentExists(LINUX):
            system("sudo rm -rf {}/DarkFly && sudo rm -rf {}/{}".format(
                LINUX,LINUX,DARKFLYEXEC
            ))
        elif self.environmentExists(TERMUX):
            system("rm -rf {}/DarkFly && rm -rf {}/{}".format(
